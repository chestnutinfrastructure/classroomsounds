...
==============================================
  ...
  Version: 08/05/26
  VERSION = 2026.05.08.5091

  Changes since 2026.05.08.5090:
    🌙 DORMANT MODE = LEDS OFF (was: cyan breathing pulse).
       Reported in the field — when a unit is Online — Dormant
       (school closed), staff want the LED ring physically dark,
       not glowing blue/cyan. Reasoning: dormant means "out of
       hours, no monitoring", and a glowing fixture in an empty
       classroom both wastes power and confuses cleaners /
       caretakers into thinking the device is "on".

       Implementation: standbyCyanPulse() (the breathing-cyan
       renderer used exclusively for the !schoolOpen branch in
       handleLEDs()) is replaced by standbyLedsOff() which writes
       all pixels to 0 at a self-healing 4Hz cadence (~15x less
       strip work than v5090's 60Hz pulse, and still reasserts
       dark if any other render path leaks a frame). Watchdog
       reset preserved (dormant can run for hours/days). No
       other behavioural change — POST scheduling, heartbeats,
       OTA window all untouched.

       Bench-test: set school closed in admin, confirm LED ring
       goes dark within ~1 second; confirm device still reports
       to dashboard every ~3 minutes (heartbeat cadence).

  Changes since 2026.05.05.5085:
    🧹 CLEAN MARK — no behavioural changes from v5085.
       Issued as a fresh baseline so the manifest, the binary,
       the embedded FW_VERSION and the GitHub release tag all
       agree from a known-good starting point. Cuts the Lyppard
       OTA bootloop where manifest version (2026.05.07.5080)
       drifted from the binary's embedded FW_VERSION
       (2026.05.05.5085) and devices kept re-downloading the
       "newer" file forever.

       From this release on, use release-firmware.py — it
       extracts FW_VERSION from this source, names the .bin
       Holly_RDiQ5000_<ver>.bin, creates the GitHub release, and
       rewrites manifest.json so its `version` field MATCHES the
       embedded FW_VERSION exactly. No more drift, no more
       bootloops.

       Bench-test before fleet rollout: flash one Holly, confirm
       boot serial prints `FW_VERSION 2026.05.08.5090`, then on
       the next 20:00–21:00 window the OTA check should say
       "up to date".
...
//        OTA
// =====================
static const char* FW_VERSION = "2026.05.08.5091";
...
void breathingBlue();
void standbyLedsOff();   // v5091: was standbyCyanPulse — dormant now renders all-off, see header changelog.
...
void standbyLedsOff() {
  ...
  // v5091: Dormant = strip dark. Self-healing 4Hz refresh — cheap (~15x
  // less work than the v5090 60Hz cyan-pulse renderer) and reasserts the
  // off-state if any other render path (active monitoring, locate, etc.)
  // wrote pixels before schoolOpen flipped back to false.
  static unsigned long lastUpdate = 0;
  unsigned long nowMs = millis();
  if (nowMs - lastUpdate < 250) return;
  lastUpdate = nowMs;
  for (int i = 0; i < NUM_LEDS; i++) strip.setPixelColor(i, 0);
  strip.show();
}
...
void loop() {
  ...
  if (!schoolOpen) {
    standbyLedsOff();   // v5091: was cyan pulse — dormant is now strip-dark.
    return;
  }
