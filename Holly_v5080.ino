...
//        OTA
// =====================
static const char* FW_VERSION = "2026.05.04.5080";
...
void updateMicFloorTest() {
  ...
  float db = readMicDB();
  if (!isfinite(db) || db < 10.0f || db > 90.0f) return;
  micFloorSamples++;
  int maxIdx = 0;
...
Thresholds computeCalibratedThresholdsFromStats(const char* band) {
  ...
  if (variance < 0.0) variance = 0.0; double sd = sqrt(variance);
  Thresholds th;
  if (sd < 1.0) { th.greenMax=(float)(calMean+1.0)-2.0f; th.amberMax=(float)(calMean+3.0); th.redWarnDb=(float)(calMean+6.0); }
  else { th.greenMax=(float)(calMean+0.90*sd)-2.0f; th.amberMax=(float)(calMean+1.60*sd); th.redWarnDb=(float)(calMean+2.40*sd); }
...
void finalizeRoomCalibrationIfDue(time_t nowEpoch) {
  ...
  if ((uint32_t)(nowEpoch - roomCalStartEpoch) < ROOM_CAL_SECONDS) return;
...
void addHealthPayload(DynamicJsonDocument& doc) {
  ...
  doc["last_zone"]=getCurrentLEDState(); doc["firmware_version"]=FW_VERSION; doc["wifi_rssi"]=WiFi.RSSI();
...
void processRemoteConfig(const String& responseBody) {
  ...
  nvsConfigVersion=serverVersion; preferences.putULong("cfg_version",nvsConfigVersion);
  remoteConfigVersionPending=serverVersion;  // flag for main loop to print
}
...
void logToGoogleSheets(float avgDB) {
  ...
  if(roomCalEnabled&&!roomCalComplete&&hasValidTime(roomCalStartEpoch)){time_t now=time(NULL);if(hasValidTime(now)){
    uint32_t el=(uint32_t)(now-roomCalStartEpoch);int pct=constrain((int)((float)el/(float)ROOM_CAL_SECONDS*100.0f),0,100);
    char ps[8];snprintf(ps,sizeof(ps),"%d%%",pct);doc["learning_progress"]=ps;}}
...
void setup() {
  ...
  if (micFloorValid && !roomCalEnabled && !roomCalComplete) {
    roomCalEnabled = true;
    preferences.putBool("rc_enabled", true);
...
void loop() {
  ...
  if(deviceMode==MODE_HOLLY_HOP){if(hollyHopEndAt!=0&&nowMs>=hollyHopEndAt)beginExitHollyHop(nowMs,"timer");
    if(deviceMode==MODE_HOLLY_HOP){renderSleepingRainbow();return;}}
  ...
  {
    const bool inActiveMonitoring = (ws == LESSON) || (!useTimetable);
